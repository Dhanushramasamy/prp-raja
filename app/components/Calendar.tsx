    ...
    try {
      ...
      data?.forEach((item) => {
        existingDataMap[item.set_number as SetNumberType] = {
          ...
          direct_sales_normal: item.direct_sales_normal,
          direct_sales_doubles: item.direct_sales_doubles,
          direct_sales_small: item.direct_sales_small,
          sales_breakage: item.sales_breakage ?? 0,
          set_breakage: item.set_breakage ?? 0,
          mortality: item.mortality,
          culls_in: item.culls_in,
          in_count: item.in_count ?? 0,
          normal_wb: item.normal_wb ?? 0,
          doubles_wb: item.doubles_wb ?? 0,
          small_wb: item.small_wb ?? 0,
          vaaram: item.vaaram,
          virpanaiyalar: item.virpanaiyalar || '',
        };
      });
